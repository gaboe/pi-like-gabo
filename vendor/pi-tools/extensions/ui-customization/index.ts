...
export function arrangeFooterStatuses(
  usage: string,
    ...
    ctx.ui.setFooter((tui, theme, footerData: ReadonlyFooterDataProvider) => {
      ...
      return {
        ...
        render(width: number) {
          ...
          const statusLayout = arrangeFooterStatuses(
          ...
          const model = modelInfo.provider
            ...
            : modelInfo.modelId;
          const session = `session ${ctx.sessionManager.getSessionId()}`;
          ...
          const lines = [
            ...
            columns(statusLayout.usage, theme.fg("muted", git), width),
          ];

          for (const statusLine of statusLayout.remaining) {
            lines.push(
