 ...
 *
 * While ≥1 process runs, a one-line widget below the fleet section shows
...
import { provideBackgroundTerminalService } from "./src/api.ts";
...
export default function (pi: ExtensionAPI) {
  ...
  let resolvedManager: TerminalManagerShape | undefined;
  let sessionContext: ExtensionContext | undefined;
  let ui: ExtensionUIContext | undefined;
  let unsubStatus: (() => void) | undefined;
  let unsubSettled: (() => void) | undefined;
  let serviceLifetime:
    ReturnType<typeof provideBackgroundTerminalService> | undefined;
  ...
  const getManager = () => {
    const managerRuntime = getRuntime();
    managerPromise ??= managerRuntime
      .runPromise(TerminalManager)
      .then((manager) => {
        resolvedManager = manager;
        // A shutdown can win while runtime service resolution is queued.
        if (runtime !== managerRuntime) return manager;
        unsubSettled?.();
        ...
        serviceLifetime = provideBackgroundTerminalService({
          start: (options) => runTool(managerRuntime, manager.start(options)),
          status: (id) => runTool(managerRuntime, manager.status(id)),
          list: () => runTool(managerRuntime, manager.list),
          kill: (ids) => runTool(managerRuntime, manager.kill(ids)),
          subscribeSettled: (listener) =>
            manager.view.subscribeSettled(listener),
          subscribeOutput: (listener) => manager.view.subscribeOutput(listener),
        });
  ...
  pi.events.on(AUTOMATION_PAUSE_CHANNEL, (value) => {
    ...
    if (!request || request.reason !== "double-escape" || !resolvedManager)
      return;
    const ids = resolvedManager.view
      .list()
      .filter((snap) => snap.status === "running")
      .map((snap) => snap.id);
    if (ids.length === 0) return;
    request.acknowledge("background-terminals", ids.length);
    void runTool(getRuntime(), resolvedManager.kill(ids)).catch((error) =>
      console.error("background-terminals: global pause failed", error),
    );
  });
  ...
  const onSettled = (snap: TerminalSnapshot, consumed: boolean) => {
    emitTelemetry(pi.events, backgroundTerminalTelemetry(snap, "settle"));
    if (consumed) {
  ...
  pi.on("session_start", (_event, ctx) => {
    ...
    void getManager().catch((error) => {
      console.error("background-terminals: failed to initialize", error);
    });
  });
  ...
  pi.registerTool({
    ...
    async execute(_toolCallId, params, _signal, _onUpdate, ctx) {
      ...
      emitTelemetry(pi.events, backgroundTerminalTelemetry(snap, "start"));
