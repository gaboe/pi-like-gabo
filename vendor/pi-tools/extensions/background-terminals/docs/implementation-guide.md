...
>   the installed `@earendil-works/pi-coding-agent/docs/` directory
...
├── src/
│   ├── api.ts                # effect-free sibling-extension service + ready rendezvous
...
Note: we do **not** need `@effect/platform-node`. Subagents use Pi in-process sessions; this guide uses raw
...
`index.ts` builds the runtime eagerly so the sibling-extension API becomes ready even when no
tool runs, and disposes it on `session_shutdown`:
...
const getManager = () => {
  managerPromise ??= getRuntime()
    ...
    .then((manager) => {
      unsubSettled = manager.view.subscribeSettled(onSettled);
      unsubStatus?.();
...
Per effect-v4-extension-guide.md §0 the async core is Effect; per the subagent manager precedent
...
- **Plain TS callbacks:** `child.stdout.on("data")`, `child.on("exit")` handlers mutate the
  entry snapshot and call `notify()` directly. Do NOT build a `Queue<SubagentEvent>`/pump-fiber pipeline here: a single command spawn does not need one.

## 6. Node child_process design (the core of `start`)

Model on this extension's existing command lifecycle (spawn options, kill-tree, and terminate-with-escalation):
...
- **Global mutation protocol.** Reserve process-global shell activity synchronously before `spawn`. An active workspace-mutation lease rejects the start before process creation. Retain activity through process settlement or kill/disposal cleanup, then release it exactly once; permanent shell taint remains even after failed spawn because detached descendants cannot be disproved.
...
On settle, the manager invokes every listener registered through
`view.subscribeSettled`. The extension's own listener:
