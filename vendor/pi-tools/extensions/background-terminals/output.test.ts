...
import { createBoundedSpillSink, OutputBuffer } from "./src/output.ts";
...
test("bounded spill sink stops disk writes at the byte limit", async () => {
  let output = "";
  let limitCount = 0;
  ...
  const sink = createBoundedSpillSink(target, 5, () => limitCount++);
  sink.write("abc");
  sink.write("def");
  sink.write("ignored");
