...
import { createBoundedSpillSink, OutputBuffer } from "./output.ts";
import {
...
interface Entry {
  ...
  child: ChildProcess;
  shellActivity: WorkspaceActivity;
...
export interface StartOptions {
  ...
  readonly cwd: string;
  /** Optional live copies. The manager honors Writable backpressure. */
...
export interface TerminalReadModel {
  ...
  /** `consumed` means an active bg_kill is returning the result itself. */
  subscribeSettled(
    listener: (snap: TerminalSnapshot, consumed: boolean) => void,
  ): () => void;
  /** Raw decoded stdout/stderr chunks, before retention trimming. */
...
const makeManager = Effect.gen(function* () {
  ...
  >();
  const outputListeners = new Set<(event: TerminalOutputChunk) => void>();
  const outputNotifyTimers = new Map<string, ReturnType<typeof setTimeout>>();

  const notify = (id?: string, broad = true) => {
    if (broad)
  ...
  const scheduleOutputNotify = (id: string) => {
    if (outputNotifyTimers.has(id)) return;
    outputNotifyTimers.set(
      id,
      setTimeout(() => {
        outputNotifyTimers.delete(id);
        notify(id, false);
      }, 50),
    );
  };
  const flushOutputNotify = (id: string) => {
    const timer = outputNotifyTimers.get(id);
  ...
  const settle = (entry: Entry) => {
    ...
    // Settlement is a boundary: flush pending output with this immediate update.
    flushOutputNotify(s.id);
    notify(s.id);
    // During teardown, don't queue results into a shutting-down session.
    for (const listener of disposed ? [] : [...settledListeners]) {
      try {
        listener(s, consumed);
      } catch {
  ...
  const makeSpill = (
    ...
    try {
      ...
      const sink = createBoundedSpillSink(
        file,
      ...
      file.on("error", fail);
      return { spillPath, file, sink, fail };
    } catch {
      return undefined;
    }
  };
  ...
  const connectOutput = (
    ...
    const states = new Set<SinkState>();
    const resumeIfReady = () => {
      if (
        [...states].every((state) => !state.blocked) &&
        !source.destroyed &&
    ...
    source.on("data", (chunk: string) => {
      buffer.push(chunk);
      const event: TerminalOutputChunk = {
      ...
      for (const state of [...states]) {
        try {
          if (!state.sink.write(chunk) && states.has(state)) {
            state.blocked = true;
            state.sink.once("drain", state.onDrain);
            source.pause();
          }
        } catch (error) {
          detach(state, error);
      ...
      // Detail views repaint at most 20fps; raw listeners still receive each chunk.
      scheduleOutputNotify(entry.snapshot.id);
    });
    source.once("end", () => {
      for (const state of [...states]) detach(state);
    });
  };

  const start = (options: StartOptions) =>
    Effect.gen(function* () {
      let shellActivity: WorkspaceActivity | undefined;
      // Reserve synchronously before spawn so parallel starts cannot race the
      // cap or an exclusive workspace lease.
      yield* Effect.suspend(
        (): Effect.Effect<void, SpawnError | ConcurrencyLimitError> => {
          ...
          try {
            shellActivity = beginGlobalShellExecution();
          } catch (error) {
            return new SpawnError({ message: boundedError(error) });
          }
          reserved++;
          return Effect.void;
        },
      ...
      const doStart = Effect.gen(function* () {
        ...
        // setEncoding's StringDecoder is multibyte-safe across chunks. A
        // false sink/spill write pauses this readable until every drain;
        // failed sinks detach so one consumer cannot deadlock the process.
        if (child.stdout) {
          connectOutput(entry, "stdout", child.stdout, stdoutBuf, [
            ...(stdoutSpill
              ? [
                  {
                    sink: stdoutSpill.sink,
                    fail: stdoutSpill.fail,
                  },
                ]
              : []),
            ...(options.stdoutSink ? [{ sink: options.stdoutSink }] : []),
          ]);
        }
        if (child.stderr) {
          connectOutput(entry, "stderr", child.stderr, stderrBuf, [
            ...(stderrSpill
              ? [
                  {
                    sink: stderrSpill.sink,
                    fail: stderrSpill.fail,
                  },
                ]
              : []),
            ...(options.stderrSink ? [{ sink: options.stderrSink }] : []),
          ]);
        }
  ...
  const disposeAll = Effect.gen(function* () {
    ...
    yield* Effect.sync(() => {
      for (const timer of outputNotifyTimers.values()) clearTimeout(timer);
      outputNotifyTimers.clear();
      const dir = spillDir;
      spillDir = null;
