/** Pi in-process backend interface. */
...
export interface SubagentSession {
   ...
   */
  /** Start the initial prompt. Backend allocation itself stays paused. */
  readonly start: Effect.Effect<void, SendError>;
  send(text: string): Effect.Effect<void, SendError>;
  enterHandoffOnly?(): void;
  /**
   * Interrupt the active run. Resolves after authoritative terminal run state
   * or completed process/session termination. Callers bound this with a
   * timeout and fall back to closing the session scope.
   */
