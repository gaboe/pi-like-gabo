...
for (const model of ["openai-codex/gpt-6-luna", "openai-codex/gpt-6-sol"])
...
test("TODO completion review uses the allowed canonical Luna model", () => {
  ...
  assert.ok(match);
  assert.equal(match[1], "openai-codex/gpt-6-luna");
...
test("resolved Pi child models reject every non-tier provider/model", () => {
  for (const model of ["anthropic/claude-sonnet", "openai-codex/gpt-6-other"])
