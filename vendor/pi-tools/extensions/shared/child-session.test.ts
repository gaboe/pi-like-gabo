...
test("child sessions exclude extensions that own parent TODO state", () => {
  const todo = { tools: new Map([["todo", {}]]) };
  const safe = { tools: new Map([["fixture", {}]]) };

  const result = excludeSessionOwnedChildExtensions({
    extensions: [todo, safe],
  });

  assert.deepEqual(result.extensions, [safe]);
});
