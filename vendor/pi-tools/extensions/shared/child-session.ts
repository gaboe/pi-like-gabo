...
const SESSION_OWNED_CHILD_TOOLS = ["todo"] as const;

export function excludeSessionOwnedChildExtensions<
  T extends { extensions: Array<{ tools: { has(name: string): boolean } }> },
>(base: T): T {
  return {
    ...base,
    extensions: base.extensions.filter(
      (extension) =>
        !SESSION_OWNED_CHILD_TOOLS.some((name) => extension.tools.has(name)),
    ),
  };
}
