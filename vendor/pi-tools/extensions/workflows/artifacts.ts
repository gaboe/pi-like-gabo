...
type ArtifactSources = {
  agents: { agent: object; transcript: TranscriptEntry[] }[];
  result: unknown;
};

const artifactSources = new WeakMap<WorkflowDetails, ArtifactSources>();

function transcriptsUnchanged(
  previous: ArtifactSources | undefined,
  details: WorkflowDetails,
) {
  return (
    previous?.agents.length === details.agents.length &&
    details.agents.every(
      (agent, index) =>
        previous.agents[index]?.agent === agent &&
        previous.agents[index]?.transcript === agent.transcript,
    )
  );
}

export function persistWorkflowJson(runDir: string, details: WorkflowDetails) {
  const previous = artifactSources.get(details);
  const unchangedTranscripts = transcriptsUnchanged(previous, details);
  if (!unchangedTranscripts) {
  ...
  if (details.result !== undefined && previous?.result !== details.result) {
    writeRunFile(
      runDir,
      "result.json",
