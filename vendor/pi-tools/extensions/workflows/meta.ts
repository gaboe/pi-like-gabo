...
function unavailableGlobalError(name: string): never {
  throw new Error(
    `workflow sandbox does not expose \`${name}\`; pass data through workflow args or use a literal`,
  );
}

function validateUnavailableGlobals(program: Program): void {
  const visit = (node: Node, parent?: Node): void => {
    if (node.type === "ImportExpression") {
      throw new Error("workflow scripts cannot use dynamic imports");
    }
    if (node.type === "Identifier") {
      const identifier = node as Identifier;
      if (UNAVAILABLE_GLOBALS.has(identifier.name)) {
        const unary =
          parent?.type === "UnaryExpression"
            ? (parent as UnaryExpression)
            : undefined;
        const property =
          parent?.type === "Property" ? (parent as Property) : undefined;
        const member =
          parent?.type === "MemberExpression"
            ? (parent as MemberExpression)
            : undefined;
        const allowedTypeCheck =
          unary?.operator === "typeof" && unary.argument === node;
        const propertyName =
          property?.key === node && !property.computed && !property.shorthand;
        const memberName = member?.property === node && !member.computed;
        if (!allowedTypeCheck && !propertyName && !memberName)
          unavailableGlobalError(identifier.name);
      }
    }
        ...
        const name =
          !member.computed && member.property.type === "Identifier"
            ...
            : member.computed &&
              ...
              : undefined;
        if (name && UNAVAILABLE_GLOBALS.has(name)) unavailableGlobalError(name);
      }
    }
...
export function prepareWorkflowScript(source: string): PreparedWorkflowScript {
  ...
  validateUnavailableGlobals(program);
