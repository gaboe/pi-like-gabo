...
function normalizeDetails(
  ...
  for (const item of rawAgents) {
    ...
    agents.push({
      ...
      maxTurns:
        typeof a.maxTurns === "number" && Number.isInteger(a.maxTurns)
          ? a.maxTurns
          : undefined,
      maxContinuations:
        typeof a.maxContinuations === "number" &&
        Number.isInteger(a.maxContinuations)
          ? a.maxContinuations
          : undefined,
      continuationsUsed:
        typeof a.continuationsUsed === "number" &&
...
function loadRunIds(directory = runsDir()): string[] {
  try {
    return fs.readdirSync(directory).filter((name) => name.startsWith("wf_"));
  } catch {
    return [];
  }
}
...
export function loadRunEntries(
  ...
  historical = new Map<string, RunEntry | undefined>(),
  directory = runsDir(),
  knownRunIds?: readonly string[],
): RunEntry[] {
  const names = new Set(knownRunIds ?? loadRunIds(directory));
  for (const runId of active.keys()) names.add(runId);
  ...
  for (const runId of names) {
    ...
    if (!historical.has(runId)) {
      let entry: RunEntry | undefined;
      try {
        const raw = JSON.parse(
          fs.readFileSync(path.join(directory, runId, "workflow.json"), "utf8"),
        );
        ...
        if (
          ...
          entry = { runId, details, live: false };
        }
      } catch {
        // Skip unreadable runs.
      }
      historical.set(runId, entry);
    }
    const entry = historical.get(runId);
...
export class WorkflowDashboard {
  ...
  private close: () => void;
  private historical = new Map<string, RunEntry | undefined>();
  ...
  constructor(
    ...
    if (initialRunId) {
      ...
      if (entry) {
        ...
        this.view = "detail";
        if (initialAgentIndex !== undefined) {
          const groups = phaseGroups(entry.details, true);
          const phaseIndex = groups.findIndex((group) =>
            group.agents.some((agent) => agent.index === initialAgentIndex),
          );
          if (phaseIndex >= 0) {
            this.phaseIndex = phaseIndex;
            this.agentIndex = groups[phaseIndex].agents.findIndex(
              (agent) => agent.index === initialAgentIndex,
            );
  ...
  private renderList(width: number, height: number): string[] {
    ...
    lines.push(
      this.hintLine(
        `${this.keys("tui.select.up")}/${this.keys("tui.select.down")} select · ${this.keys("tui.select.confirm")} open · r reload · ${this.keys("tui.select.cancel")} close`,
        width,
      ),
    );
    return lines;
