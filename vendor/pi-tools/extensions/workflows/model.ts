...
export const RESERVED_HANDOFF_TURNS = 2;

export function turnBudgetBreakdown(totalTurns: number) {
  const total = Math.max(0, Math.floor(totalTurns));
  const reserve = Math.min(total, RESERVED_HANDOFF_TURNS);
  return { total, usable: total - reserve, reserve };
}
...
export function formatUsage(
  ...
  if (maxTurns !== undefined) {
    ...
    if (maxContinuations > 0) {
      parts.push(`${usage.turns} turns used`);
      parts.push(`${maxTurns} total/attempt`);
      parts.push(`${budget.usable} work + ${budget.reserve} handoff/attempt`);
    } else {
      parts.push(`${usage.turns}/${maxTurns} turn${maxTurns === 1 ? "" : "s"}`);
      if (maxTurns > 1) {
        parts.push(`${budget.usable} work + ${budget.reserve} handoff`);
      }
    }
  } else if (usage.turns) {
