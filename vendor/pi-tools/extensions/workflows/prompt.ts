...
export const WORKFLOW_PROMPT_GUIDELINES = [
  ...
  "Use `openai-codex/gpt-5.6-luna` for bounded low-risk read-only work or precisely scoped implementation when expected behavior, an existing pattern, and a deterministic check are explicit. Never use it for domain/security/money/database/migration/concurrency/production decisions; escalate to Terra when the work requires architecture choices, inferred domain behavior, or coupled changes. A failed check stays at the same tier unless it exposes missing reasoning or scope ambiguity.",
  "Set every workflow child's model and effort independently: use Luna for focused scouts/verifiers, broad exploration, and precisely scoped low-risk implementation; set Luna effort low for mechanical work, medium for multi-step reasoning, or high only when bounded work is genuinely difficult. Use Terra low for ambiguous root causes, domain decisions, or broad/coupled multi-file implementation, Sol low for routine review, and Sol medium for planning or complex synthesis.",
