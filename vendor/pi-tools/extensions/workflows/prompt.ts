...
export const WORKFLOW_TOOL_DESCRIPTION = [
  ...
  "Set each child's `cwd` explicitly when the workflow targets a different trusted worktree; do not merely embed an absolute path in the prompt. Pass a `schema` to agent() whenever a later step branches on the result. Audit, review, and verification agents must use a status schema containing `done`/`passed` plus `blocked` or `failed`; plain `.ok` means the model call completed, not that the reviewed implementation passed. Route bounded low-risk read-only work or precisely scoped implementation to `model: 'openai-codex/gpt-6-luna'` when expected behavior, an existing pattern, and deterministic verification are explicit. Choose effort independently by reasoning depth. Size `maxTurns` explicitly as work turns plus 2 (`4` work turns require `maxTurns: 6`); otherwise split or rescope instead of silently growing the cap. Escalate ambiguity or scope that requires architecture choices, inferred domain behavior, or coupled changes; a failed check stays at the same tier unless it exposes missing reasoning or scope ambiguity. For resumable local work, include `status: done|partial|blocked|failed`, use `maxContinuations: 1`, and reserve `blocked` for a real prerequisite or contradiction. A continuation verifies the shared worktree diff before proceeding; it never grants external mutation permission. Artifacts are saved under ~/.pi/agent/workflows/<runId>/ for inspection.",
...
export const WORKFLOW_PROMPT_GUIDELINES = [
  ...
  "Use `openai-codex/gpt-6-luna` for bounded low-risk read-only work or precisely scoped implementation when expected behavior, an existing pattern, and a deterministic check are explicit. Never use it for domain/security/money/database/migration/concurrency/production decisions; escalate to Luna medium for coupled changes, or to Sol when the work requires architecture choices or inferred domain behavior. A failed check stays at the same tier unless it exposes missing reasoning or scope ambiguity.",
  "Set every workflow child's model and effort independently: use Luna for focused scouts/verifiers, broad exploration, and precisely scoped low-risk implementation; set Luna effort low for mechanical work, medium for multi-step reasoning, or high only when bounded work is genuinely difficult. Use Luna medium for ambiguous root causes or broad/coupled multi-file implementation, Sol low for routine review and domain decisions, and Sol medium for planning or complex synthesis.",
