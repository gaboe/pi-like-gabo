...
test("sandbox forwards per-agent deadline, turn limit, and continuation bound", async () => {
  let timeoutMs: unknown;
  let maxTurns: unknown;
  let maxContinuations: unknown;
  let cwd: unknown;
  await run(
    `return await agent("bounded", { cwd: "/tmp/worktree", timeoutMs: 1234, maxTurns: 4, maxContinuations: 1 });`,
    {
  ...
  assert.equal(maxTurns, 4);
  assert.equal(maxContinuations, 1);
