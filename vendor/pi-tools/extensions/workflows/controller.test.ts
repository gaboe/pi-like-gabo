...
test("continuation reservations consume the same global call budget", async () => {
  const controller = new RunController();
  for (let index = 0; index < MAX_AGENT_CALLS; index++)
    controller.reserveCall();
  assert.equal(controller.calls, MAX_AGENT_CALLS);
  assert.throws(() => controller.reserveCall(), /exceeded the limit/);
  assert.equal(await controller.settle(), true);
});
