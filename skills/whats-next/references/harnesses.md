...
- Reviewer: one tool-free Luna child, low effort, `maxTurns: 1`, bounded timeout, no extensions.
...
- CLI fallback: `codex exec -C "$ROOT" -m gpt-6-luna -s read-only -c 'model_reasoning_effort="low"' --json -o "$OUT" "$PROMPT"`, with a 15-minute hard cap and non-empty prompt guard.
