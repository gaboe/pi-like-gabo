...
Use workflow only on explicit request. Set child `cwd` to trusted checkout/worktree. Choose model and effort independently. Model: use `openai-codex/gpt-6-luna` for focused scouts/verifiers and precisely scoped low-risk implementation with an existing pattern and deterministic check, `openai-codex/gpt-6-luna` for ambiguous root causes, domain decisions, or broad/coupled implementation, and `openai-codex/gpt-6-sol` for review/planning. Effort: use low for mechanical work, medium for multi-step reasoning, and high only when the selected model's bounded work is genuinely difficult. Use Sol high for security, concurrency, money, migrations, or other irreversible risk; use xhigh only after documented high-effort insufficiency. Estimate orientation, edits, focused verification, and handoff before assigning `max_turns`: 8–12 narrow scouts, 16–24 broad review/planning, 24–32 focused implementation, 32–48 one cohesive multi-file/root-cause package. Reserve final two turns; keep trivial work small rather than over-fanning out. A worker may settle before reserve with exact `budget_request` only for bounded remaining work. Parent may approve it once through `subagent_send`, up to 48 total turns. Parent runs long checks as jobs.
