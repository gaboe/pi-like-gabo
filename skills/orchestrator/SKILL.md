...
**Check the floor before the first phase.** Orchestration has a lower bound, and below it the ceremony costs more than it buys. Work one worker could finish inside its own budget is not a program. A published head-to-head on this pattern (Cursor's `pstack` orchestrate playbook) reports a twelve-unit job landing one unit under a full coordinator and all twelve under a plain agent; the number is theirs and unverified here, but the shape matches what durable phases, seed contracts, and a gate per result actually cost. Below the floor, do the work directly, verify inline, and skip the machinery. Say out loud that you are below the floor instead of collapsing the phases silently, because a half-applied contract reads as a followed one.
