...
function sink(stream: TerminalOutput["stream"], onOutput: (event: TerminalOutput) => void) {
	return new Writable({
		decodeStrings: false,
		write(chunk: string | Buffer, _encoding, callback) {
			try {
				onOutput({ stream, data: typeof chunk === "string" ? chunk : chunk.toString("utf8") });
				callback();
			} catch (error) {
...
export class BackgroundTerminalAdapter {
	...
	private async getService() {
		...
		return await new Promise<BackgroundTerminalService>((resolve, reject) => {
			...
			const timer = setTimeout(() => {
				...
				reject(new Error("Background terminal service did not become ready within 5 seconds."));
			}, 5_000);
	...
	async start(
		...
		const snapshot = await service.start({ ...options, stdoutSink, stderrSink });
