...
} from "./wait-registration-service.js";
...
type BoundWaitRegistration = WaitRegistration & { incarnation: string };

const MAX_WAIT_QUERY_IDS = 64;
const MAX_WAIT_REGISTRATIONS = 1_024 * 64;

function waitRegistrationKey(registration: WaitRegistration): string {
...
function waitRegistrationQueryKey(registration: WaitRegistration): string {
  return JSON.stringify([
    registration.id,
    registration.waitToken,
    registration.registeredAt,
...
export class WaitRegistrationStore {
  private registrations = new Map<string, BoundWaitRegistration>();

  register(
    registration: WaitRegistration,
    job: JobRecord,
  ): BoundWaitRegistration | undefined {
    const key = waitRegistrationKey(registration);
    const existing = this.registrations.get(key);
    if (existing)
      return existing.incarnation === job.incarnation ? existing : undefined;
    if (
      (registration.incarnation !== undefined &&
        registration.incarnation !== job.incarnation) ||
      (registration.incarnation === undefined && registration.bind !== true)
    )
      return undefined;
    const bound = { ...registration, incarnation: job.incarnation };
    delete bound.bind;
    if (this.registrations.size >= MAX_WAIT_REGISTRATIONS) return undefined;
    this.registrations.set(key, bound);
    return bound;
  }

  retain(registrations: readonly WaitRegistration[]): void {
    const keys = new Set(registrations.map(waitRegistrationKey));
    for (const key of this.registrations.keys())
      if (!keys.has(key)) this.registrations.delete(key);
  }

  replace(registrations: readonly BoundWaitRegistration[]): void {
    if (registrations.length > MAX_WAIT_REGISTRATIONS)
      throw new Error("Too many wait registrations.");
    this.registrations = new Map(
      registrations.map((registration) => [
        waitRegistrationKey(registration),
        registration,
      ]),
    );
  }
  ...
  forJob(job: Pick<JobRecord, "id" | "incarnation">): BoundWaitRegistration[] {
...
export function stateEvent(
  job: Pick<JobRecord, "id" | "status" | "settledAt" | "error">,
  wake = false,
  registration?: Pick<
    WaitRegistration,
    "waitToken" | "registeredAt" | "generation" | "incarnation"
  >,
): JobStateEvent {
  const status = wake
    ? "wake"
...
export function installJobSessionListeners(
  ...
  handlers: {
    ...
    registerWait(value: unknown): void;
    fleetQuery(): void;
  ...
  const stops = [
    ...
    events.on("jobs:wait-registration", handlers.registerWait),
...
export const ACTIVATION_TEARDOWN_TIMEOUT_MS = 1_000;
...
export default function (pi: ExtensionAPI) {
  type Activation = {
    ...
    adapter: BackgroundTerminalAdapter;
    delivery: FollowupDelivery;
    manager?: JobManager;
    stopSessionListeners?: () => void;
    stopWaitRegistrationService?: () => void;
    dispose?: Promise<void>;
    ready?: Promise<JobManager>;
  ...
  const waitRegistrations = new WaitRegistrationStore();

  const isCurrent = (activation: Activation) =>
    active === activation && activationEpoch === activation.epoch;
  const disposeActivation = (activation: Activation) =>
    (activation.dispose ??= (async () => {
      activation.controller.abort(new Error("Jobs activation superseded."));
      activation.stopWaitRegistrationService?.();
      activation.stopSessionListeners?.();
      activation.delivery.close();
      activation.adapter.dispose();
      if (activation.connecting)
        await settlesWithin(
      ...
      if (!manager) return;
      if (activation.initializing)
        await settlesWithin(
          activation.initializing,
          ACTIVATION_TEARDOWN_TIMEOUT_MS,
        );
      await settlesWithin(
        manager.dispose({ persist: false }),
        ACTIVATION_TEARDOWN_TIMEOUT_MS,
      );
    })());

  const parseWaitRegistrations = (
    ...
    for (const registration of registrations) {
      ...
      if (
        ...
        !entry.id ||
        typeof entry.waitToken !== "string" ||
        !entry.waitToken ||
        entry.waitToken.length > 128 ||
        ...
        !Object.hasOwn(entry, "waitToken") ||
        !Object.hasOwn(entry, "registeredAt") ||
        !Object.hasOwn(entry, "generation") ||
        (Object.hasOwn(entry, "incarnation") &&
          (typeof entry.incarnation !== "string" || !entry.incarnation)) ||
        (Object.hasOwn(entry, "bind") && entry.bind !== true) ||
        (entry.incarnation === undefined && entry.bind !== true)
      )
        return undefined;
      const parsedRegistration: WaitRegistration = {
        id: entry.id,
        waitToken: entry.waitToken,
        registeredAt: entry.registeredAt,
        generation: entry.generation as number,
        ...(typeof entry.incarnation === "string"
          ? { incarnation: entry.incarnation }
          : {}),
        ...(entry.bind === true ? { bind: true } : {}),
      };
      const key = waitRegistrationKey(parsedRegistration);
  ...
  const validateWaitQuery = (
    ids: unknown,
    registrations: unknown,
  ):
    | { requested: ReadonlySet<string>; registrationKeys: ReadonlySet<string> }
    | undefined => {
    if (
      !Array.isArray(ids) ||
      ids.length > MAX_WAIT_QUERY_IDS ||
      ids.some((id) => typeof id !== "string" || !id)
    )
      return undefined;
    const requested = new Set<string>(ids);
    if (requested.size > MAX_WAIT_QUERY_IDS) return undefined;
    const parsed = parseWaitRegistrations(registrations);
    if (
      !parsed ||
      parsed.some((registration) => !requested.has(registration.id))
    )
      return undefined;
    return {
      requested,
      registrationKeys: new Set(parsed.map(waitRegistrationQueryKey)),
    };
  };
  const registerWait = (registrations: unknown): JobStateEvent[] => {
    const valid = parseWaitRegistrations(registrations);
    if (!valid) return [];
    const states: JobStateEvent[] = [];
    for (const registration of valid) {
      const job = manager
        ?.list()
        .find((candidate) => candidate.id === registration.id);
      if (!job) continue;
      const bound = waitRegistrations.register(registration, job);
      if (bound) states.push(stateEvent(job, false, bound));
    }
    return states;
  };
  const syncWaitRegistrations = (
    registrations: readonly JobWaitRegistration[],
  ): JobStateEvent[] | undefined => {
    const valid = parseWaitRegistrations(registrations);
    if (!valid || !manager) return undefined;
    const jobs = new Map(manager.list().map((job) => [job.id, job]));
    const bound: BoundWaitRegistration[] = [];
    const states: JobStateEvent[] = [];
    for (const registration of valid) {
      const job = jobs.get(registration.id);
      if (
        !job ||
        (registration.incarnation !== undefined &&
          registration.incarnation !== job.incarnation)
      ...
      bound.push(next);
      states.push(stateEvent(job, false, next));
    }
    waitRegistrations.replace(bound);
    return states;
  };

  const queryWait = (
    ids: readonly string[],
    requestedRegistrations: readonly JobWaitRegistration[] = [],
  ): JobStateEvent[] => {
    if (!manager) return [];
    const query = validateWaitQuery(ids, requestedRegistrations);
    if (!query) return [];
    return manager
      .list()
      .filter((job) => query.requested.has(job.id))
      .flatMap((job) => {
        const registrations = waitRegistrations
          .forJob(job)
          .filter((registration) =>
            query.registrationKeys.has(waitRegistrationQueryKey(registration)),
          );
        return registrations.map((registration) =>
          stateEvent(job, false, registration),
        );
      });
  };
  ...
  const handleQuery = (value: unknown) => {
    ...
    const query = validateWaitQuery(request.ids, request.registrations ?? []);
    if (!query) {
      request.respond([]);
      return;
    }
      ...
      manager
        ...
        .flatMap((job) => {
          const registrations = waitRegistrations
            .forJob(job)
            .filter((registration) =>
              query.registrationKeys.has(
                waitRegistrationQueryKey(registration),
              ),
            );
  ...
  const getManager = async () => {
    ...
    if (!activation?.ready || managerReady !== activation.ready)
      throw new Error("Jobs extension has not received session_start yet.");
    const currentManager = await managerReady;
    if (!isCurrent(activation) || manager !== currentManager)
      throw new Error(
        "Jobs extension session changed while waiting for startup.",
      );
  ...
  pi.on("session_start", async (_event, ctx) => {
    ...
    active = activation;
    previous?.controller.abort(new Error("Jobs activation superseded."));
    waitRegistrations.clear();
    context = undefined;
    manager = undefined;
    managerReady = undefined;
    delivery = undefined;

    activation.ready = (async () => {
      try {
        if (previous) await disposeActivation(previous);
        if (!isCurrent(activation)) throw new Error("Jobs startup superseded.");
        try {
          activation.connecting = activation.adapter.connect(
            activation.controller.signal,
          );
          await activation.connecting;
        } catch (error) {
          if (!isCurrent(activation)) throw error;
          ctx.ui.notify(
        ...
        if (!isCurrent(activation)) throw new Error("Jobs startup superseded.");
        const localManager = new JobManager(
          new JobStore(undefined, activation.controller.signal),
          activation.adapter,
          {
            onLifecycle(event) {
              if (!isCurrent(activation) || activation.manager !== localManager)
                return;
              ...
              if (
                ...
                const registrations = waitRegistrations.forJob({
                  id: event.jobId,
                ...
                for (const registration of registrations.length
                  ? registrations
                  : [undefined]) {
              ...
              activation.delivery.push(event);
              updateStatus();
            },
            async approveNetwork(request) {
              if (!isCurrent(activation) || ctx.mode !== "tui" || !ctx.hasUI)
                return false;
              ...
              return (
                isCurrent(activation) && manager === localManager && approved
              );
            },
          },
        );
        activation.manager = localManager;
        activation.initializing = localManager.initialize(
          ctx.sessionManager.getSessionId(),
          ctx.cwd,
          activation.controller.signal,
        );
        await activation.initializing;
        if (!isCurrent(activation)) throw new Error("Jobs startup superseded.");
        activation.stopSessionListeners = installJobSessionListeners(
          pi.events,
          {
            query: handleQuery,
            registerWait: () => {},
        ...
        activation.stopWaitRegistrationService =
          registerJobsWaitRegistrationService({
            ...
            sync: syncWaitRegistrations,
            query: queryWait,
          });
        if (!isCurrent(activation)) throw new Error("Jobs startup superseded.");
        manager = localManager;
        managerReady = activation.ready;
        delivery = activation.delivery;
        context = ctx;
        updateStatus();
        return localManager;
      } finally {
        if (!isCurrent(activation)) void disposeActivation(activation);
      }
    })();
    try {
  ...
  pi.on("session_shutdown", async () => {
    ...
    if (context === activation.ctx && context.mode === "tui")
      context.ui.setStatus("jobs", undefined);
    publishFleetState(pi.events, { source: "jobs", items: [] });
    context = undefined;
    manager = undefined;
    managerReady = undefined;
    delivery = undefined;
    await disposeActivation(activation);
  });
