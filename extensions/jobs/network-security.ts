...
function ipRestriction(address: string) {
  ...
  if (normalized === "::" || normalized === "::1")
    return normalized === "::1" ? "loopback IPv6" : "unspecified IPv6";
  if (normalized.startsWith("fc") || normalized.startsWith("fd"))
    return "private IPv6";
  if (/^fe[89ab]/.test(normalized)) return "link-local IPv6";
  if (normalized.startsWith("ff")) return "multicast IPv6";
  if (normalized.startsWith("::ffff:"))
    return ipv4Restriction(normalized.slice(7));
  return undefined;
}

export async function inspectNetworkTarget(
  raw: string,
  protocols: readonly string[],
): Promise<NetworkApproval> {
  let url: URL;
  try {
    url = new URL(raw);
  } catch {
  ...
  if (!protocols.includes(url.protocol))
    throw new Error(`URL scheme must be ${protocols.join(" or ")}.`);
  if (url.username || url.password)
    throw new Error("Credentials in network URLs are not allowed.");
  ...
  if (
    ["localhost", "localhost.localdomain", "metadata.google.internal"].includes(
      hostname.toLowerCase(),
    )
  )
    restricted.add("local/metadata hostname");
  if (!isIP(hostname)) {
  ...
  return {
    endpoint,
    hostname,
    addresses: [...new Set(resolvedAddresses)].sort(),
