...
function throwIfAborted(signal?: AbortSignal) {
  if (signal?.aborted)
    throw signal.reason ?? new Error("Job activation cancelled.");
}

function isPersistedScope(
  value: unknown,
  sessionId: string,
  cwd: string,
): value is PersistedScope {
...
export class JobStore {
  ...
  constructor(
    root = join(homedir(), ".pi", "agent", "jobs"),
  ...
  async open(sessionId: string, cwd: string) {
    ...
    try {
      ...
      throwIfAborted(this.signal);
      await rename(this.file, `${this.file}.invalid-${Date.now()}`).catch(
        () => undefined,
      );
      return undefined;
    } catch (error) {
  ...
  async append(record: JobRecord, event: JobEvent) {
    ...
    const tail = content.subarray(
      Math.max(0, content.length - RETAINED_LOG_BYTES),
    );
