...
test("canonical path analysis handles existing, nonexistent, symlink, and escape targets", () => {
  ...
  assert.equal(
    analyzePath(pathToFileURL(path.join(root, "file.txt")).href, root)
      .containment,
    "inside",
  );
  assert.equal(
    analyzePath("~/permissions-home-test", root).targetExists,
    false,
  );
});
...
test("audit event is bounded and omits secrets and command payloads", () => {
  ...
  const secret =
    "TOKEN=super-secret curl https://example.test/private?q=secret";
  const event = createAuditEvent(
    "bash",
    { command: secret, apiKey: "hidden", path: "new.txt" },
    root,
  );
...
test("observe-only hook never returns a blocking decision and recorder stays bounded", () => {
  const emitted = [];
  const recorder = createBoundedAuditRecorder(
    (event) => emitted.push(event),
    2,
  );
  const result = observeToolCall(
...
test("Pi tool_call handler observes and returns undefined", async () => {
  ...
  const result = handlers.get("tool_call")(
    { toolName: "bash", input: { command: "secret" } },
    { cwd: process.cwd() },
  );
  assert.equal(result, undefined);
  ...
  assert.equal(
    throwingHandlers.get("tool_call")(
