...
function transcriptCategory(name: string): ToolCategory | "mutation" {
  ...
  return base === "edit" || base === "write"
    ? "mutation"
    : (categoryFor(name) ?? "other");
}

type RenderEntry = CompactToolEntry & {
  summary?: string;
...
function aggregate(
  ...
  if (entries.length === 1)
    return (
      entries[0].summary ??
      resultSummary(category, entries[0].output ?? "", entries[0].running)
    );
  if (category !== "read" && category !== "search") return "";
  const total = entries.reduce(
    (sum, entry) =>
      sum + (entry.outputLineCount ?? nonEmptyLineCount(entry.output ?? "")),
    0,
  );
...
function renderRow(
  ...
  const suffix = [
    ...
    duration(entries),
    category === "command" ? "Ctrl+O expand" : "",
  ]
    .filter(Boolean)
    .join(" · ");
  const lines = [
    `${marker} ${theme.fg("muted", limit(suffix ? `${body} · ${suffix}` : body, width - 2))}`,
  ];
  if (category === "command") {
...
function mutationChanges(
  ...
  for (const edit of edits) {
    ...
    for (const line of String(
      value.newText ?? value.new_string ?? value.content ?? "",
    ).split("\n")) {
      if (line) changes.push({ prefix: "+", text: line });
    }
  }
...
function renderMutation(
  ...
  const marker = entry.isError
    ...
    : entry.running
      ...
      : theme.fg("success", "◆");
  const lines = [
    `${marker} ${theme.fg("toolTitle", `${humanize(base)} ${path}`)}`,
  ];
  ...
  if (!changes.length) {
    ...
    if (summary)
      lines[0] += theme.fg(
        "dim",
        ` · ${limit(summary, Math.max(10, width - path.length - 10))}`,
      );
    return lines;
  }
  ...
  for (const change of changes.slice(0, 10)) {
    ...
    lines.push(
      `${theme.fg("dim", "│ ")}${theme.fg(color, plain)}${" ".repeat(Math.max(0, inner - 1 - plain.length))}${theme.fg("dim", "│")}`,
    );
  }
  if (changes.length > 10) {
    const omitted = `… ${changes.length - 10} more lines`;
    lines.push(
      `${theme.fg("dim", "│ ")}${theme.fg("muted", omitted)}${" ".repeat(Math.max(0, inner - 1 - omitted.length))}${theme.fg("dim", "│")}`,
    );
  }
  lines.push(theme.fg("dim", `╰${"─".repeat(inner)}╯`));
  return lines;
...
export function renderCompactEntries(
  ...
  for (let index = 0; index < entries.length;) {
    ...
    if (category === "mutation") {
      out.push(
        ...renderMutation(entry, options.cwd, options.width, options.theme),
      );
      index++;
      continue;
    }
    ...
    if (category === "read" || category === "search") {
      while (
        index + group.length < entries.length &&
        transcriptCategory(entries[index + group.length].name) === category
      ) {
        group.push(entries[index + group.length]);
      }
    }
    out.push(
      ...renderRow(group, category, options.cwd, options.width, options.theme),
    );
    index += group.length;
  }
  return out;
