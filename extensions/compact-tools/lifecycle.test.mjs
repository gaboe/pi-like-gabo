...
import { getCompactToolRenderer, setCompactToolRenderer } from "../../vendor/pi-tools/extensions/shared/compact-tool-renderer-protocol.ts";
...
test("restores renderer globals when the extension runtime shuts down", () => {
  let shutdown;
  compactTools(extensionApi((handler) => { shutdown = handler; }));
...
test("does not tear down a newer compact-tools runtime", () => {
  ...
  const externalRenderer = { version: 1, enabled: () => true, render: () => [] };
  setCompactToolRenderer(externalRenderer);
  let shutdownA;
  let shutdownB;
  compactTools(extensionApi((handler) => { shutdownA = handler; }));
  compactTools(extensionApi((handler) => { shutdownB = handler; }));
