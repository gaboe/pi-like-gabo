...
test("removes workspace prefixes from paths and commands", () => {
	assert.equal(compactPath(`${cwd}/extensions/jobs/manager.ts`, cwd), "extensions/jobs/manager.ts");
	assert.equal(displayTarget("read", "read", { path: `${cwd}/extensions/jobs/manager.ts` }, cwd), "extensions/jobs/manager.ts");
	assert.deepEqual(commandDisplay({ command: `cd ${cwd} && bun run check` }, cwd), {
		command: "bun run check",
		location: ".",
	});
	assert.equal(commandTarget({ command: `cd ${cwd} && bun run check` }, cwd), ". · bun run check");
});
...
test("summarizes results without exposing full output", () => {
	const result = { content: [{ type: "text", text: "a.ts:1\na.ts:2\nb.ts:8\n" }] };
	assert.equal(resultSummary("search", result), "3 results");
	assert.equal(resultSummary("command", { content: [{ type: "text", text: "82 pass\n0 fail" }] }), "82 tests passed");
	assert.equal(resultSummary("other", { content: [{ type: "text", text: "Updated task #4\nlarge payload" }] }), "Updated task #4");
});

test("shows concise custom-tool intent and bounded errors", () => {
	assert.equal(customTarget("todo", { action: "update", id: 4 }), "Todo update");
	assert.equal(customTarget("workflow", { name: "review" }), "Workflow review");
	assert.deepEqual(
		errorPreview({ content: [{ type: "text", text: "first\nsecond\nthird\nfourth" }] }),
		["second", "third", "fourth"],
	);
});
