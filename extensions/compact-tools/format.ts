...
const LABEL_KEYS = ["action", "title", "subject", "query", "pattern", "name"] as const;
...
function firstString(args: unknown, keys: readonly string[]): string {
	...
	for (const key of keys) {
		if (typeof record[key] === "string" && record[key]) return oneLine(record[key]);
	}
	return "";
}
...
export function commandDisplay(args: unknown, cwd: string): { command: string; location: string } {
...
export function commandTarget(args: unknown, cwd: string): string {
	...
	return display.location ? `${display.location} · ${display.command}` : display.command;
}
...
export function resultSummary(category: ToolCategory, result: unknown, partial = false): string {
...
export function displayTarget(category: ToolCategory, name: string, args: unknown, cwd: string): string {
	switch (category) {
		case "read": return readTarget(args, cwd);
		case "search": return searchTarget(args);
		case "command": return commandTarget(args, cwd);
		case "other": return customTarget(name, args);
	}
}
