...
type OwnedEntry = { name: string; path: string; bytes: number; mtime: number; artifact: string | undefined };
type BatchEntry = { indexes: number[]; identity: string; data: Buffer; artifact: Artifact };

async function withDirectoryLock<T>(directory: string, task: () => Promise<T>): Promise<T> {
	...
	try {
		...
		release();
		if (directoryLocks.get(directory) === queued) directoryLocks.delete(directory);
	}
}

export type Artifact = { id: string; path: string; sha256: string; bytes: number };
...
export async function artifactLocation(sessionDir: string, sessionId: string): Promise<ArtifactLocation | undefined> {
	...
	try {
		...
		if (dirname(directory) !== root || !directory.startsWith(`${root}${sep}`)) return undefined;
...
export class ToolResultArtifacts {
	...
	constructor(readonly directory: string, options: { root?: string; hooks?: ArtifactHooks } = {}) {
	...
	async persistBatch(items: readonly { identity: string; text: string }[]): Promise<(Artifact | undefined)[]> {
		try {
			return await withDirectoryLock(this.directory, async () => {
				...
				for (const [index, { identity, text }] of items.entries()) {
					...
					unique.set(key, {
						...
						identity,
						data,
						artifact: { id, path: join(this.directory, `${id}.txt`), sha256, bytes: data.length },
					});
				}
				...
				for (const [key, entry] of unique) {
					try {
						...
						if ((error as NodeJS.ErrnoException).code === "ENOENT") candidates.push(entry);
					}
				}

				const retained = await this.ownedEntries();
				const reusablePaths = new Set([...reusable.values()].map(({ path }) => path));
				const fixed = retained.filter((entry) => !entry.artifact || reusablePaths.has(entry.path));
				...
				for (const candidate of candidates) {
					...
					if (fixedBytes + reservedBytes + candidate.data.length > MAX_ARTIFACT_TOTAL_BYTES) continue;
				...
				for (const [key, artifact] of reusable) {
					for (const index of unique.get(key)!.indexes) results[index] = artifact;
				}
				for (const entry of selected) {
					const artifact = await this.persistOne(entry.identity, entry.data.toString("utf8"));
					if (artifact) for (const index of entry.indexes) results[index] = artifact;
				}
				return await Promise.all(results.map(async (artifact) => artifact && await this.verify(artifact) ? artifact : undefined));
			});
		} catch {
	...
	private async persistOne(identity: string, text: string): Promise<Artifact | undefined> {
		const data = Buffer.from(text, "utf8");
	...
	private async writeOrReuse(key: string, data: Buffer, sha256: string): Promise<Artifact | undefined> {
		...
		try {
			...
			if (!info.isFile() || info.isSymbolicLink() || !await this.verify(artifact)) return undefined;
		...
		const temp = join(this.directory, `.compact-tools-${id}-${randomBytes(8).toString("hex")}.tmp`);
		...
		try {
			...
			try {
				...
				if (!existing.isFile() || existing.isSymbolicLink() || !await this.verify(artifact)) return undefined;
				this.byKey.set(key, artifact);
				return artifact;
			} catch (error) {
				if ((error as NodeJS.ErrnoException).code !== "ENOENT") return undefined;
	...
	private async verify(artifact: Artifact): Promise<boolean> {
		...
		try {
			...
			if (data.length !== artifact.bytes || createHash("sha256").update(data).digest("hex") !== artifact.sha256) return false;
			await file.chmod(0o600);
			const secured = await file.stat();
			return this.safeFile(secured, artifact.bytes) && (secured.mode & 0o777) === 0o600;
		} catch {
			return false;
		} finally {
			await file?.close().catch(() => undefined);
	...
	private safeFile(info: Awaited<ReturnType<Awaited<ReturnType<typeof open>>["stat"]>>, bytes: number): boolean {
		if (!info.isFile() || info.size !== bytes) return false;
		if (typeof process.getuid === "function" && typeof info.uid === "number" && info.uid !== process.getuid()) return false;
		if (typeof info.nlink === "number" && info.nlink !== 1) return false;
		return true;
	}

	private async ensureDirectory() {
		if (this.directory !== this.root && dirname(this.directory) !== this.root) throw new Error("artifact directory escapes root");
		if (this.directory === this.root) {
			await mkdir(this.directory, { recursive: true, mode: 0o700 });
		} else {
			await mkdir(this.root, { mode: 0o700 }).catch((error: NodeJS.ErrnoException) => {
	...
	private async secureDirectory(path: string) {
		const info = await lstat(path);
		if (!info.isDirectory() || info.isSymbolicLink()) throw new Error("artifact directory is unsafe");
	...
	async cleanup(preferredPaths: readonly string[] = []): Promise<ArtifactCleanup> {
	...
	private async ownedEntries(): Promise<OwnedEntry[]> {
		const entries = await Promise.all((await readdir(this.directory)).map(async (name) => {
			if (!ARTIFACT_NAME.test(name) && !TEMP_NAME.test(name)) return undefined;
			const path = join(this.directory, name);
			const info = await lstat(path).catch(() => undefined);
			if (!info?.isFile() || info.isSymbolicLink()) return undefined;
			if (TEMP_NAME.test(name) && Date.now() - info.mtimeMs >= STALE_TEMP_MS) {
				await rm(path, { force: true });
				return undefined;
			}
			return { name, path, bytes: info.size, mtime: info.mtimeMs, artifact: ARTIFACT_NAME.exec(name)?.[1] };
		}));
		return entries.filter((entry): entry is OwnedEntry => !!entry);
	}

	private async makeRoom(preferredPaths: readonly string[], reservedCount: number, reservedBytes: number): Promise<ArtifactCleanup> {
		...
		const artifacts = retained.filter((entry) => entry.artifact).sort((left, right) => {
		...
		for (const entry of artifacts) {
			if (count + reservedCount <= MAX_ARTIFACT_COUNT && total + reservedBytes <= MAX_ARTIFACT_TOTAL_BYTES) break;
		...
		if (deleted.paths.length) {
			...
			for (const [key, artifact] of this.byKey) if (paths.has(artifact.path)) this.byKey.delete(key);
		}
		return deleted;
	}
}
