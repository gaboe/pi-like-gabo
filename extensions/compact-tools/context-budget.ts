...
export const NON_TEXT_PROVIDER_PLACEHOLDER_CHARS = "(see attached image)"
  .length;
export const MAX_NON_TEXT_BLOCKS_PER_RESULT = 16;
const NO_TOOL_OUTPUT_CHARS = "(no tool output)".length;
const OMITTED_TOOL_RESULT =
  "[Older tool output omitted from LLM context; full output remains in transcript]";
...
export function modeledProviderChars(content: readonly unknown[]): number {
  ...
  return (
    (text.length || attachments ? text.length : NO_TOOL_OUTPUT_CHARS) +
    attachments * NON_TEXT_PROVIDER_PLACEHOLDER_CHARS
  );
}
...
function boundNonText(content: readonly unknown[]): {
  content: unknown[];
  omitted: number;
  ...
  if (attachments <= MAX_NON_TEXT_BLOCKS_PER_RESULT)
    return { content: [...content], omitted: 0, originalText };
  let seen = 0;
  let inserted = false;
  ...
  const output = content.flatMap((block) => {
    ...
    return [
      {
        type: "text",
        text: `[${omitted} image/binary blocks omitted from LLM context; binary data is not persisted]`,
        [GENERATED_OMISSION]: true,
      } satisfies TextBlock,
    ];
  });
...
function rebuildText(
  ...
  const sourceBlocks = content.filter(
    (block) => isTextBlock(block) && originalText.has(block as object),
  ) as TextBlock[];
  const first = sourceBlocks[0];
  const last = sourceBlocks.at(-1);
  let insertedHead = false;
  return content.flatMap((block) => {
    if (!isTextBlock(block) || !originalText.has(block as object))
      return [block];
...
function boundToolResultContextAt<T extends ToolResult>(
  message: T,
  recovery: RecoveryMarker | undefined,
  index: number,
): T {
  ...
  if (
    !boundedNonText.omitted &&
    modeledProviderChars(message.content) <= MAX_TOOL_RESULT_TEXT_CHARS
  )
    return message;

  const source = providerText(message.content);
  if (
    modeledProviderChars(boundedNonText.content) <= MAX_TOOL_RESULT_TEXT_CHARS
  ) {
...
export function boundToolResultContext<T extends ToolResult>(
  message: T,
...
export function boundToolResultHistory<T extends ToolResult>(
  messages: readonly T[],
  recovery?: RecoveryMarker,
): T[] {
  const individuallyBounded = messages.map((message, index) =>
    boundToolResultContextAt(message, recovery, index),
  );
  const output = messages.map((message) => {
    ...
    return replaceToolResult(
      message,
      nonTextCount(message.content) ? "[output omitted]" : "…",
    );
  });
  let used = output.reduce(
    (total, message) =>
      ...
      (message.role === "toolResult" && Array.isArray(message.content)
        ? modeledProviderChars(message.content)
        : 0),
    0,
  );
  ...
  for (
    let index = 0;
    used > MAX_TOOL_HISTORY_TEXT_CHARS && index < output.length;
    index++
  ) {
    ...
    used +=
      modeledProviderChars(replacement.content as unknown[]) -
      modeledProviderChars(message.content);
  ...
  for (let index = output.length - 1; index >= 0; index--) {
    ...
    const delta =
      modeledProviderChars(candidate.content) -
      modeledProviderChars(current.content);
    if (delta <= room) {
  ...
  if (used > MAX_TOOL_HISTORY_TEXT_CHARS) {
    return output.filter((message) => {
      if (
        used <= MAX_TOOL_HISTORY_TEXT_CHARS ||
        message.role !== "toolResult" ||
        !Array.isArray(message.content)
      )
        return true;
