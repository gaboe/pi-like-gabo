...
test("subagent takeover uses the shared renderer for grouped reads and detailed edits", () => {
  ...
  const transcript = [
      ...
      parts: [
        {
          ...
          toolId: "r1",
          name: "read",
          argsPreview: JSON.stringify({ path: "/tmp/worktree/src/a.ts" }),
        },
      ],
    },
    {
      ...
      parts: [
        {
          ...
          toolId: "r2",
          name: "read",
          argsPreview: JSON.stringify({ path: "/tmp/worktree/src/b.ts" }),
        },
      ],
    },
    {
    ...
    },
    {
      ...
      name: "edit",
      isError: false,
      outputPreview: "Successfully replaced 1 block",
    },
  ];
...
test("subagent live tools use shared compact rows", () => {
      ...
      [
        {
          toolId: "s1",
          name: "grep",
          argsPreview: JSON.stringify({ pattern: "JobManager" }),
          outputPreview: "a.ts:1\nb.ts:2",
        },
        {
          toolId: "s2",
          name: "grep",
          argsPreview: JSON.stringify({ pattern: "dispose" }),
          outputPreview: "b.ts:4",
        },
      ],
    ),
    100,
  ...
  assert.match(
    output,
    /◇ Search 2 queries · "JobManager" · "dispose" · 3 results/,
  );
});
...
test("bash rows compact the worktree but keep the full command and expansion hint", () => {
  ...
  const output = renderCompactEntries(
    [
      {
        ...
        name: "bash",
        args: {
          command: `cd /tmp/worktrees/core-332-nsure-lock-role && ${command}`,
        },
    ...
    { cwd: "/tmp/worktree", width: 100, theme },
  ).join("\n");
  assert.match(
    output,
    /Bash · core-332-nsure-lock-role · 1895 tests passed · Ctrl\+O expand/,
  );
...
test("long shell tokens wrap by terminal width instead of one character per row", () => {
  ...
  assert.equal(
    lines.some((line) => /^\s+x\s*$/.test(line)),
    false,
  );
});
