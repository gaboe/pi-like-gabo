...
test("registers context budgeting independently of compact display state", async () => {
	...
	assert.ok(
		result.messages[0].content[0].text.length <= MAX_TOOL_RESULT_TEXT_CHARS,
	);
});
...
const historyToolChars = (messages) =>
	messages
...
test("bounds aggregate tool history while preserving recent results and transcript source", () => {
	...
	assert.deepEqual(
		bounded.map(({ toolCallId, toolName, isError, details }) => ({
			toolCallId,
			toolName,
			isError,
			details,
		})),
		messages.map(({ toolCallId, toolName, isError, details }) => ({
			toolCallId,
			toolName,
			isError,
			details,
	...
	assert.match(
		bounded[1].content.find((block) => block.type === "text").text,
		/omitted|…/,
	);
	assert.match(
		bounded.at(-1).content.find((block) => block.type === "text").text,
		/^39:/,
	);
...
const omitted = (content) =>
	Number(
		content
			.find(
				(block) => block.type === "text" && /chars omitted/.test(block.text),
			)
			.text.match(/\[(\d+) chars omitted/)[1],
	);
...
test("counts Pi 0.82.1 OpenAI text separators and exact source omission", () => {
	const source = {
		...
		content: [
			...
			{ type: "text", text: "y".repeat(8192) },
		],
	};
	assert.equal(
		convertToLlm([source])[0]
	...
	assert.equal(
		omitted(bounded.content),
		providerText(source.content).length -
			bounded.content
				.filter(
					(block) => block.type === "text" && !/chars omitted/.test(block.text),
				)
	...
	assert.equal(
		omitted(emptyBounded.content),
		providerText(separatorsOnly.content).length -
			emptyBounded.content
				.filter(
					(block) => block.type === "text" && !/chars omitted/.test(block.text),
				)
				.reduce((size, block) => size + block.text.length, 0),
	...
	const sparse = {
		role: "toolResult",
		content: [
			...separatorsOnly.content.slice(0, -1),
			{ type: "text", text: "x" },
		],
	};
	...
	assert.equal(
		omitted(sparseBounded.content),
		providerText(sparse.content).length -
			sparseBounded.content
				.filter(
					(block) => block.type === "text" && !/chars omitted/.test(block.text),
				)
				.reduce((size, block) => size + block.text.length, 0),
	...
	const fragmented = {
		role: "toolResult",
		content: Array.from({ length: 16_386 }, () => ({
			type: "text",
			text: "x",
		})),
	};
	...
	assert.ok(
		providerChars(fragmentedBounded.content) <= MAX_TOOL_RESULT_TEXT_CHARS,
	);
	assert.equal(
		omitted(fragmentedBounded.content),
		providerText(fragmented.content).length -
			fragmentedBounded.content
				.filter(
					(block) => block.type === "text" && !/chars omitted/.test(block.text),
				)
				.reduce((size, block) => size + block.text.length, 0),
	...
	assert.equal(
		omitted(sliced.content),
		providerText(twoLarge.content).length -
			sliced.content
				.filter(
					(block) => block.type === "text" && !/chars omitted/.test(block.text),
				)
...
test("reports exact original provider character ranges for 20,002 characters", () => {
	const source = {
		role: "toolResult",
		content: [{ type: "text", text: "x".repeat(20_002) }],
	};
	let individual;
	...
	boundToolResultHistory(
		[
			...
			...Array.from({ length: 5 }, (_, index) => ({
				role: "toolResult",
				content: [{ type: "text", text: `${index}${"z".repeat(20_001)}` }],
			})),
		],
		(_message, info, index) => {
...
test("caps 4,000 image-only blocks and preserves latest bounded images", () => {
	const source = {
		role: "toolResult",
		content: Array.from({ length: 4_000 }, (_, index) => ({
			type: "image",
			data: `${index}`,
			mimeType: "image/png",
		})),
	...
	assert.deepEqual(
		...
		Array.from(
			{ length: MAX_NON_TEXT_BLOCKS_PER_RESULT },
			(_, index) => `${4_000 - MAX_NON_TEXT_BLOCKS_PER_RESULT + index}`,
		),
	);
	assert.match(
		bounded.content.find((block) => block.type === "text").text,
		/3984 image\/binary blocks omitted/,
	);
	assert.ok(providerChars(bounded.content) <= MAX_TOOL_RESULT_TEXT_CHARS);
	assert.ok(historyToolChars([bounded]) <= MAX_TOOL_HISTORY_TEXT_CHARS);

	const repeated = { type: "image", data: "same", mimeType: "image/png" };
	const seventeen = boundToolResultContext({
		role: "toolResult",
		content: Array(17).fill(repeated),
	});
	assert.equal(
		seventeen.content.filter((block) => block.type === "image").length,
		MAX_NON_TEXT_BLOCKS_PER_RESULT,
	);
});
...
test("preserves interleaved block order, source, and success/error caps", () => {
	const source = {
		role: "toolResult",
		content: [
			{ type: "text", text: `head${"a".repeat(20_000)}` },
			{ type: "image", data: "one", mimeType: "image/png" },
			{ type: "text", text: `${"b".repeat(20_000)}tail` },
	...
	assert.ok(providerChars(bounded.content) <= MAX_TOOL_RESULT_TEXT_CHARS);
	assert.deepEqual(
		bounded.content.filter((block) => block.type === "image"),
		source.content.filter((block) => block.type === "image"),
	);
	assert.match(
		bounded.content.filter((block) => block.type === "text")[0].text,
		/^head/,
	);
	assert.match(
		bounded.content.filter((block) => block.type === "text").at(-1).text,
		/tail$/,
	);
	const error = boundToolResultContext({ ...source, isError: true });
	assert.ok(providerChars(error.content) <= MAX_TOOL_RESULT_TEXT_CHARS);
	assert.ok(
		error.content.filter((block) => block.type === "text").at(-1).text.length >=
			bounded.content.filter((block) => block.type === "text").at(-1).text
				.length,
	);
});
...
test("empty in-memory session disables persistence and keeps normal truncation", async () => {
	...
	const existed = await lstat(cwdArtifacts).then(
		() => true,
		() => false,
	);
	const result = await context(
	...
	const marker = result.messages[0].content.find(
		(block) => block.type === "text" && /chars omitted/.test(block.text),
	).text;
	assert.doesNotMatch(marker, /Recovery id=/);
	assert.equal(
		await lstat(cwdArtifacts).then(
			() => true,
			() => false,
		),
		existed,
...
test("duplicate tool-call IDs bind recovery to message index and exact content", async (t) => {
	...
	const paths = result.messages.map(
		(message) =>
			message.content
				.find(
					(block) =>
						block.type === "text" && block.text.includes("Recovery id="),
				)
				.text.match(/path=(\S+)/)[1],
	);
	...
	assert.ok(
		paths.every(
			(path) =>
				path.startsWith(`${canonicalRoot}/.compact-tool-artifacts/`) &&
...
test("batch retention emits only exact artifacts still present at handoff", async (t) => {
	const { lstat, mkdtemp, readFile, readdir, rm } = await import(
		"node:fs/promises"
	);
	const { tmpdir } = await import("node:os");
	const { join } = await import("node:path");
	...
	const messages = Array.from({ length: 20 }, (_, index) => ({
		...
		toolCallId: `call-${index}`,
		content: [
			{ type: "text", text: `${index}:${String(index).repeat(20_002)}` },
		],
	}));
	const result = await context(
		{ messages },
	...
	const markers = result.messages
		.flatMap((message) => message.content ?? [])
		.filter(
			(block) => block.type === "text" && block.text.includes("Recovery id="),
		);
...
test("recovery markers preserve UTF-8 multiblock output and reuse aggregate artifacts", async (t) => {
	...
	const messages = [
		{
			role: "toolResult",
			toolCallId: "utf8-call",
			content: [
				{ type: "text", text: `α\n${"x".repeat(10_000)}` },
				{ type: "image", data: "not-persisted" },
				{ type: "text", text: `β\n${"y".repeat(10_000)}` },
			],
		},
		...Array.from({ length: 5 }, (_, index) => ({
			...
			content: [{ type: "text", text: `${index}:${"z".repeat(14_000)}` }],
		})),
	];
	const ctx = {
		sessionManager: {
	...
	assert.ok(
		[
			multiText,
			...messages.slice(1).map((message) => message.content[0].text),
		].includes(stored),
	);
	assert.match(
		marker,
		new RegExp(`sha256=${createHash("sha256").update(stored).digest("hex")}`),
	);
	...
	assert.deepEqual(
		messages[0].content.filter((block) => block.type === "image"),
		[{ type: "image", data: "not-persisted" }],
	);
	const artifactDirectory = join(
		root,
		".compact-tool-artifacts",
		createHash("sha256").update("session-test").digest("hex"),
	);
