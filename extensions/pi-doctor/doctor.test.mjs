...
function adapters(overrides = {}) {
  return {
    manifest: () => ({
      pi: {
        extensions: ["./extensions/z/index.ts", "./extensions/a/index.ts"],
      },
    }),
...
test("records and formatted output have stable deterministic ordering", () => {
  ...
  assert.match(
    ...
    /registered=manifest only; loaded=runtime provenance seen; active=callable/,
  );
});

test("isolated discovery and optional probe failures degrade only their records", () => {
  const records = collectCapabilityRecords(
    adapters({
      manifest: () => {
        ...
        commands: [
          {
            name: "pi-doctor",
            sourceInfo: {
              source: "./extensions/pi-doctor/index.ts",
              path: "/repo/extensions/pi-doctor/index.ts",
            },
          },
        ],
      }),
  ...
  assert.equal(
    records.find(({ id }) => id === "discovery:manifest")?.state,
    "degraded",
  );
  assert.equal(
    records.find(({ id }) => id.startsWith("command:pi-doctor:"))?.state,
    "active",
  );
  assert.equal(
    records.find(({ id }) => id === "optional:pi-lens")?.state,
    "degraded",
  );
});
...
test("manifest loaded and runtime active remain distinct states", () => {
  const records = collectCapabilityRecords(
    adapters({
      manifest: () => ({
        pi: { extensions: ["./extensions/pi-doctor/index.ts"] },
      }),
      runtime: () => ({
        tools: [],
        commands: [
  ...
  assert.equal(
    records.find(({ kind }) => kind === "extension")?.state,
    "loaded",
  );
      ...
      tools: [
        {
          name: "off",
          active: false,
          sourceInfo: { source: "off", path: "off" },
        },
      ],
      commands: [],
    })[0].state,
...
test("provider and dependency failures have isolated diagnostics", () => {
  ...
  assert.equal(
    records.find(({ id }) => id === "provider:selected")?.state,
    "degraded",
  );
  assert.equal(
    records.find(({ id }) => id === "dependency:fd")?.diagnostics[0].code,
    "probe-missing",
  );
  assert.equal(
    records.find(({ id }) => id === "dependency:rg")?.diagnostics[0].code,
    "probe-failed",
  );
  assert.equal(
    records.find(({ id }) => id === "dependency:git")?.state,
    "active",
  );
});
...
test("post-dedupe Pi 0.82 runtime reports collision detection unavailable", () => {
  const base = (source) => ({
    name: "same",
    sourceInfo: { source, path: source },
  });
  const records = discoverRuntimeRecords({
    tools: [base("one"), base("two")],
    commands: [],
  });
  assert.ok(records.every(({ collisions }) => collisions.length === 0));
  assert.equal(
    records.find(({ id }) => id === "discovery:runtime-collisions")
      ?.diagnostics[0].code,
...
test("pi-lens fixtures distinguish absent, configured, present, malformed, and unverifiable", () => {
  ...
  assert.equal(
    missing.remediation,
    `${PI_LENS_INSTALL} (${PI_LENS_URL}), then /reload.`,
  );

  const configured = detectPiLens({
    cwd,
    home: cwd,
    configFiles: [path.join(fixtures, "valid-global.json")],
    resolvePackage: () => {
      throw new Error("missing");
    },
  });
  ...
  const present = detectPiLens({
    cwd,
    home: cwd,
    configFiles: [],
    resolvePackage: () => packageEntry,
  });
  assert.equal(present.state, "degraded");
  assert.equal(present.detection, "present");
  assert.equal(present.installed, true);

  const malformed = detectPiLens({
    cwd,
    home: cwd,
    configFiles: [path.join(fixtures, "malformed-global.json")],
    resolvePackage: () => packageEntry,
  });
  assert.equal(malformed.state, "degraded");
  assert.equal(malformed.detection, "malformed");

  const unverifiable = detectPiLens({
    cwd,
    home: cwd,
    configFiles: [],
    resolvePackage: () => path.join(cwd, "unknown", "index.js"),
  });
  assert.equal(unverifiable.detection, "unverifiable");

  const active = detectPiLens({
    cwd,
    home: cwd,
    configFiles: [],
    runtimeSources: [packageEntry],
    resolvePackage: () => packageEntry,
  });
  ...
  const falsePositiveRuntime = detectPiLens({
    ...
    configFiles: [],
    runtimeSources: [
      "/plugins/not-pi-lens/index.js",
      "/plugins/pi-lens-extra/index.js",
    ],
    resolvePackage: () => {
      throw new Error("missing");
    },
  ...
  const exactRuntime = detectPiLens({
    cwd,
    home: cwd,
    configFiles: [],
    runtimeSources: ["/plugins/pi-lens/index.js"],
    resolvePackage: () => {
      throw new Error("missing");
    },
  });
  ...
  writeFileSync(
    ...
    JSON.stringify({
      packages: [
        "not-pi-lens",
        "npm:pi-lens-extra",
        `git:github.com/${PI_LENS_OWNER}/not-pi-lens`,
      ],
      extensions: ["pi-lens"],
    }),
  );
  const falsePositiveSettings = detectPiLens({
    cwd,
    home: settingsHome,
    configFiles: [],
    resolvePackage: () => {
  ...
  writeFileSync(
    settingsFile,
    JSON.stringify({
      packages: [{ source: `git:github.com/${PI_LENS_OWNER}/pi-lens` }],
    }),
  );
  const exactSettings = detectPiLens({
    cwd,
    home: settingsHome,
    configFiles: [],
    resolvePackage: () => {
  ...
  writeFileSync(
    settingsFile,
    JSON.stringify({
      packages: [{ source: "git:github.com/apmantza/pi-lens" }],
    }),
  );
  const upstreamSettings = detectPiLens({
    cwd,
    home: settingsHome,
    configFiles: [],
    resolvePackage: () => {
...
test("root package pins, bundles, and activates pi-lens", () => {
  ...
  assert.deepEqual([...(manifest.bundleDependencies ?? [])].sort(), [
    "pi-caveman",
    "pi-lens",
  ]);
