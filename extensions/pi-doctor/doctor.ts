...
export type CapabilityState =
  "registered" | "loaded" | "active" | "optional" | "degraded";
...
export function sanitizeSource(value: string): string {
  ...
  try {
    const url = new URL(
      clean,
      protocolRelative ? "https://relative.invalid" : undefined,
    );
    if (!protocolRelative && !/^[a-z][a-z\d+.-]*:/i.test(clean))
  ...
  const home = homedir();
  return (
    home !== path.parse(home).root ? clean.replaceAll(home, "~") : clean
  ).slice(0, 240);
}

function stableSourceId(source: string): string {
  const normalized = source.replaceAll("\\", "/").replace(/^\.\//, "");
  const knownRoot = normalized.match(
    /(?:^|\/)((?:extensions|skills|vendor)\/.*)$/,
  );
...
function capabilityName(source: string): string {
  ...
  try {
    pathname = new URL(
      source,
      source.startsWith("//") ? "https://relative.invalid" : undefined,
    ).pathname;
  } catch {}
...
function provenance(
  input: RuntimeCapability["sourceInfo"],
  source: string,
): Provenance {
  return {
...
export function discoverManifestRecords(
  ...
  return groups.flatMap(([kind, sources]) =>
    sources.map((rawSource) => {
      ...
      return {
        ...
        diagnostics: [
          {
            code: "package-registered",
            message: `${capabilityName(source)} is declared; runtime activity is not implied.`,
          },
        ],
        collisions: [],
      };
...
export function discoverRuntimeRecords(
  ...
  const records = groups.flatMap(([kind, items]) =>
    items.map((item) => {
      ...
      return {
        ...
        diagnostics: [
          {
            code: active ? "runtime-active" : "runtime-loaded",
            message: active
              ? `${kind} ${item.name} is callable in this session.`
              : `${kind} ${item.name} is loaded but inactive.`,
          },
        ],
        collisions: [],
      };
      ...
      diagnostics: [
        {
          ...
          message:
            "Pi 0.82 runtime registries are post-deduplication/post-renaming; registration collision detection is unavailable.",
        },
      ],
      collisions: [],
    },
...
function isPiLensPackageSource(value: string): boolean {
  ...
  return /^(?:git:)?(?:(?:https?|ssh):\/\/git@|(?:https?|ssh):\/\/|git@)?github\.com[/:][^/:]+\/pi-lens(?:\.git)?(?:@[^/?#]+)?(?:[?#].*)?$/.test(
    source,
  );
}
...
export type PiLensDetection =
  "optional-absent" | "configured" | "present" | "malformed" | "unverifiable";
  ...
  while (true) {
    ...
    if (existsSync(manifestPath)) {
      try {
        ...
        ) as PackageManifest & { name?: unknown };
        const extensions = manifest.pi?.extensions;
        return manifest.name === "pi-lens" &&
...
function probeConfigFiles(files: string[]): {
  detection: "configured" | "malformed" | "unverifiable" | "optional-absent";
  present: boolean;
} {
  ...
  for (const file of files) {
    ...
    try {
      ...
      if (!parsed || typeof parsed !== "object" || Array.isArray(parsed))
        return { detection: "malformed", present: true };
      configured = true;
    } catch (error) {
      return {
        detection: error instanceof SyntaxError ? "malformed" : "unverifiable",
        present: true,
      };
    }
  }
  return configured
    ? { detection: "configured", present: true }
    : { detection: "optional-absent", present: false };
}
...
export function detectPiLens(options: PiLensProbeOptions): PiLensProbe {
  ...
  const extensionActive = (options.runtimeSources ?? []).some(
    runtimeSourceIsPiLens,
  );
  let packageDetection: "present" | "optional-absent" | "unverifiable" =
    "optional-absent";
  try {
  ...
  const settingsConfigured =
    settingsDeclarePiLens(path.join(home, ".pi", "agent", "settings.json")) ||
    settingsDeclarePiLens(path.join(options.cwd, ".pi", "settings.json"));
  const configPresent = config.present || settingsConfigured;
  const detection: PiLensDetection =
    config.detection === "malformed" || config.detection === "unverifiable"
      ? config.detection
  ...
  if (detection === "malformed")
    return {
      ...
      extensionActive,
      configPresent,
      remediation: "Fix or remove malformed pi-lens JSON config, then /reload.",
    };
  if (detection === "unverifiable")
    return {
      ...
      configPresent,
      remediation:
        "Verify pi-lens package metadata/config readability, then /reload.",
    };
  if (extensionActive)
    return {
      state: "active",
  ...
  if (detection === "present" || detection === "configured")
    return {
      state: "degraded",
      detection,
      installed,
      ...
      remediation:
        detection === "configured"
          ? `${PI_LENS_INSTALL}, then /reload.`
          : "Run /reload to activate the installed pi-lens extension.",
    };
  return {
    state: "optional",
    detection,
...
function probeRecord(
  ...
  try {
    ...
    return {
      ...
      diagnostics: available
        ...
        : [
            {
              code: "probe-missing",
              message: `${id} metadata is unavailable.`,
              remediation,
            },
          ],
      collisions: [],
    ...
    return degraded(
      ...
      kind,
      "probe-failed",
      `${id} metadata probe failed in isolation.`,
      remediation,
    );
  }
}

export function collectCapabilityRecords(
  ...
  try {
    ...
    manifestRecords = [
      degraded(
        ...
        "discovery",
        "manifest-failed",
        "Package manifest discovery failed in isolation.",
        "Check package.json readability and JSON syntax.",
      ),
    ];
  }
  ...
  try {
    ...
    runtimeRecords = [
      degraded(
        ...
        "discovery",
        "runtime-failed",
        "Runtime registry discovery failed in isolation.",
        "Run /reload; inspect extension load errors.",
      ),
    ];
  }

  const runtimeSources = runtimeRecords.map((record) =>
    stableSourceId(record.provenance.source),
  );
  manifestRecords = manifestRecords.map((record) => {
    const source = stableSourceId(record.provenance.source);
    return runtimeSources.some(
      (runtimeSource) =>
        runtimeSource.includes(source) || source.includes(runtimeSource),
    )
      ? {
          ...record,
          state: "loaded",
          diagnostics: [
            {
              code: "runtime-loaded",
              message:
                "Package declaration has runtime provenance; activity depends on its registered surface.",
            },
          ],
        }
      : record;
        ...
        ? [
            {
              ...
              message: `Selected provider: ${sanitizeSource(provider.name ?? "configured")}.`,
            },
          ]
        : [
            {
              code: "provider-missing",
              message: "No selected model provider.",
              remediation: "Select or configure a model provider.",
            },
          ],
      collisions: [],
    };
  } catch {
    providerRecord = degraded(
      "provider:selected",
      "provider",
      "provider-probe-failed",
      "Provider metadata probe failed in isolation.",
      "Inspect model provider configuration.",
    );
  }

  const dependencyRecords = (["fd", "git", "rg", "firecrawl"] as const).map(
    (name) =>
      probeRecord(
        `dependency:${name}`,
        ...
        name === "firecrawl"
          ? "Set FIRECRAWL_API_KEY in ~/.pi/agent/.env or disable Firecrawl."
          : `Install ${name} or disable its capability.`,
      ),
  );
  ...
  try {
    ...
    lensRecord = degraded(
      "optional:pi-lens",
      "optional",
      "pi-lens-probe-failed",
      "Optional pi-lens metadata probe failed in isolation; doctor remains available.",
      `${PI_LENS_INSTALL} (${PI_LENS_URL})`,
    );
  }
  ...
  return [
    ...
    (a, b) =>
      ...
      a.provenance.source.localeCompare(b.provenance.source),
  );
}

export function createDiscoveryAdapters(
  pi: {
    getAllTools(): RuntimeCapability[];
    getActiveTools(): string[];
    getCommands(): RuntimeCapability[];
  },
  packageRoot: string,
  cwd: string,
  ...
  const loadRuntime = () => {
    ...
    runtime = {
      tools: pi
        .getAllTools()
        .map((tool) => ({ ...tool, active: activeTools.has(tool.name) })),
  ...
  return {
    ...
    runtime: loadRuntime,
    provider: () => ({
      configured: Boolean(providerName),
      ...(providerName ? { name: providerName } : {}),
    }),
    dependency: (name) =>
      name === "firecrawl"
        ? Boolean(env.FIRECRAWL_API_KEY)
        : executableAvailable(name, env),
    piLens: () => {
      const current = loadRuntime();
      const runtimeSources = [...current.tools, ...current.commands].flatMap(
        (item) => [item.sourceInfo.source, item.sourceInfo.path],
      );
...
export function formatDoctorReport(records: CapabilityRecord[]): string {
  ...
  const declared = [...uniqueCollisions.values()]
    .filter(({ type }) => type === "declared-override")
    .map(({ name }) => name)
    .sort();
  const unintended = [...uniqueCollisions.values()]
    .filter(({ type }) => type === "unintended-conflict")
    .map(({ name }) => name)
    .sort();
  return [
    ...
    records.some((record) =>
      record.diagnostics.some(
        ({ code }) => code === "collision-detection-unavailable",
      ),
    )
    ...
    ...records.map((record) => {
      const diagnostics = record.diagnostics
        .map(
          (item) =>
            `${item.code}:${item.message}${item.remediation ? ` remediation=${item.remediation}` : ""}`,
        )
        .join("; ");
