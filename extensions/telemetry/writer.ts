...
export class TelemetryWriter {
	...
	write(value: unknown): Promise<void> {
		if (this.closed || this.pending >= (this.options.maxPending ?? DEFAULT_MAX_PENDING)) return Promise.resolve();
	...
	private async persist(line: string): Promise<void> {
		...
		if (!directoryStat.isDirectory() || directoryStat.isSymbolicLink()) throw new Error("Telemetry directory is not private storage.");
	...
	private async rotate(path: string, incomingBytes: number): Promise<void> {
		...
		try {
			...
			if (!current.isFile() || current.isSymbolicLink()) throw new Error("Telemetry file is not a regular file.");
			size = current.size;
		} catch (error) {
			if ((error as NodeJS.ErrnoException).code !== "ENOENT") throw error;
		}
		if (size === 0 || size + incomingBytes <= (this.options.maxBytes ?? DEFAULT_MAX_BYTES)) return;
