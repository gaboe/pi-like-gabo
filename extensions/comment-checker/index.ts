...
const binaryPath = fileURLToPath(
  new URL(
    `../../bin/${process.platform}-${process.arch}/comment-checker`,
    import.meta.url,
  ),
);
...
function checkerInput(
  ...
  cwd: string,
): CheckerInput | undefined {
  const path =
    typeof event.input.path === "string"
      ? resolve(cwd, event.input.path.replace(/^@/, ""))
      : undefined;
  if (!path) return;
...
export default function (pi: ExtensionAPI): void {
  ...
  pi.on("tool_result", async (event, ctx) => {
    ...
    const input = checkerInput(
      event as typeof event & { input: Record<string, unknown> },
      ctx.cwd,
    );
    ...
    if (!executableExists()) {
      if (!warnedMissing)
        ctx.ui.notify(
          "Comment checker binary is missing; run npm run build:comment-checker",
          "warning",
        );
      warnedMissing = true;
      return;
    ...
    return {
      content: [
        ...
        {
          type: "text",
          text: `Comment checker (${input.tool_input.file_path}):\n${warning}`,
        },
      ],
    };
  });
