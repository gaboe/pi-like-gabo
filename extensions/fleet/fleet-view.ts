...
function fleetStatusGlyph(status: FleetItem["status"]): string {
...
export function formatItemActivity(item: FleetItem): string | undefined {
	...
	return activityAt === undefined
		? undefined
		: `active ${formatAge(activityAt)} ago`;
}
			...
			item.kind === "job"
				...
				: item.kind === "subagent"
					...
					: item.kind === "workflow-run"
						? "workflow"
						: "  ↳ agent";
		const left = `  ${this.marker(index, selected, theme)} ${theme.fg(statusColor, fleetStatusGlyph(item.status))} ${theme.fg("muted", kind)}  ${item.title}${item.detail ? theme.fg("dim", ` · ${item.detail}`) : ""}`;
		const stats = [
			formatElapsed(item.startedAt, item.settledAt),
			formatItemActivity(item),
