...
describe("shared Pi and Claude memory", () => {
	it("uses Claude's encoded Nexus memory for root, descendants, and stax worktrees", () => {
		...
		assert.equal(resolveSharedMemoryFile(join(nexus, "nexus-be"), home), expected);
		assert.equal(
			resolveSharedMemoryFile(join(home, ".stax", "worktrees", "nexus-be", "fix"), home),
			expected,
		);
	});
	...
	it("bounds injected memory and requests compaction past the line limit", () => {
		...
		writeFileSync(file, Array.from({ length: MAX_MEMORY_LINES + 1 }, (_, i) => `- memory ${i}`).join("\n"));
	...
	it("keeps one stable memory snapshot for the whole session", () => {
		...
		const file = join(home, ".claude", "projects", encodeClaudeProjectPath(cwd), "memory", "MEMORY.md");
