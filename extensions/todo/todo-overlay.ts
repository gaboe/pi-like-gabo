...
const CANCELLATION_CAPACITY_WARNING =
  "⚠ cancellation recovery capacity full · resolve or re-arm existing cancellation";
const CANCELLATION_QUARANTINE_WARNING =
  "⚠ cancellation recovery quarantined · re-arm only after ownership is verified";
...
export class TodoOverlay {
  ...
  update(): void {
    ...
    if (visible.length === 0 && !snapshot.cancellationCapacityError) {
  ...
  private getSnapshot() {
    ...
    const completedTaskIds = new Set(
      state.tasks
        .filter((task) => isTaskArchivable(task))
        .map((task) => task.id),
    );
    for (const taskId of this.completedTaskIdsPendingHide) {
    ...
    return {
      tasks: [...state.tasks],
      nextId: state.nextId,
      cancellationIntents: allCancellationIntents(state),
      cancellationCapacityError: state.cancellationCapacityError,
    };
  }
  ...
  private renderWidget(theme: Theme, width: number): string[] {
    ...
    if (overlayTasks.length === 0 && !snapshot.cancellationCapacityError)
      return [];
    ...
    if (snapshot.cancellationCapacityError)
      lines.push(
        truncate(
          theme.fg(
            "warning",
            snapshot.cancellationCapacityError === CANCELLATION_QUARANTINE_ERROR
              ? CANCELLATION_QUARANTINE_WARNING
              : CANCELLATION_CAPACITY_WARNING,
          ),
        ),
      );
    if (overlayTasks.length === 0) return this.withTrailingSpacer(lines);
    const layout = selectOverlayLayout(
      overlayState,
      MAX_WIDGET_LINES - lines.length,
    );
    for (const task of layout.visible) {
      lines.push(
        truncate(
          `${theme.fg("dim", "├─")} ${formatOverlayTaskLine(task, theme, showIds, snapshot.cancellationIntents)}`,
        ),
      );
    }
