...
const SIGNALS: Array<[RegExp, string]> = [
  [
    /\b(?:orchestrat|delegate|work package|package worker|parallel package)\b/i,
    "explicit delegation",
  ],
  [
    /\b(?:plan|worktree|reload|handoff|multi[- ]repo)\b/i,
    "durable coordination",
  ],
  [/\b(?:parallel|independent)\b/i, "independent work"],
  [
    /\b(?:implement|migrate|refactor|audit)\b[\s\S]{0,160}\b(?:and|plus)\b/i,
    "broad autonomous outcome",
  ],
];

export function classifyOrchestration(
  raw: string,
  ...
  if (
    Array.isArray(prepared?.affectedPaths) &&
    prepared.affectedPaths.length > 2
  )
    signals.push("multi-path dossier");
  return {
    requiresOrchestration: signals.length > 0,
    signals: [...new Set(signals)],
  };
}

const MAX_CLASSIFIER_RESPONSE_CHARS = 2_048;

function parsedClassification(
  text: string,
): OrchestratorClassification | undefined {
  if (text.length > MAX_CLASSIFIER_RESPONSE_CHARS) return undefined;
  try {
    ...
    if (
      ...
      !Object.keys(value).every((key) =>
        ["requiresOrchestration", "signals"].includes(key),
      ) ||
      typeof (value as Record<string, unknown>).requiresOrchestration !==
        "boolean" ||
      !Array.isArray((value as Record<string, unknown>).signals)
    )
      return undefined;
    const signals = (value as Record<string, unknown>).signals as unknown[];
    if (
      signals.length > 6 ||
      !signals.every(
      ...
      return undefined;
    return {
      requiresOrchestration: (value as Record<string, boolean>)
        .requiresOrchestration,
      signals: [...new Set(signals.map((signal) => signal.trim()))],
    };
  } catch {
    return undefined;
  }
...
export function aggregateOrchestratorMode(
  ...
  return unresolved.some(
    (task) =>
      (task.metadata?.orchestrator as OrchestratorClassification | undefined)
        ?.requiresOrchestration ||
      (task.metadata?.orchestrator as { mode?: unknown } | undefined)?.mode ===
        "provisional",
  )
    ? "provisional"
    : "direct";
...
export function modeFor(
  ...
  prior?: OrchestratorMode,
): OrchestratorMode {
  if (setting === "off") return "direct";
  if (prior === "sticky") return "sticky";
  return setting === "on" || classification.requiresOrchestration
    ? "provisional"
    : "direct";
}

export function markOrchestrator(
  task: Task,
  setting: OrchestratorSetting,
  classification: OrchestratorClassification,
  phase: "raw" | "prepared",
  ...
  return {
    ...
    metadata: {
      ...
      orchestrator: {
        mode,
        phase,
        requiresOrchestration: classification.requiresOrchestration,
...
export function stickyOrchestrator(task: Task): Task {
  ...
  const prior = task.metadata?.orchestrator as
    Record<string, unknown> | undefined;
  return prior?.mode !== "provisional"
    ? task
    : {
        ...task,
        metadata: {
          ...task.metadata,
          orchestrator: { ...prior, mode: "sticky", startedAt: "execution" },
        },
      };
}
export function orchestratorStatus(
  ...
  return `orchestrator ${setting}/${aggregateOrchestratorMode(tasks, setting, sticky)}`;
}

export function orchestratorFooterStatus(
  tasks: readonly Task[],
  setting: OrchestratorSetting,
  sticky = false,
): string | undefined {
